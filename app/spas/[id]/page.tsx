                  ...
                  <div key={service.id} className="flex items-center justify-between border border-gray-100 rounded-xl p-4 hover:shadow-sm transition">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <button
                        ...
                        className="bg-[#7A9E87] text-white text-sm px-4 py-2 rounded-lg hover:bg-[#6a8e77] transition"
                      >
                        Book
                      </button>
                    </div>
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-8 relative">
            ...
            {success ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500 mb-6">We'll see you soon at {spa.name}.</p>
                <button onClick={() => setShowModal(false)} className="bg-[#7A9E87] text-white px-6 py-2 rounded-lg hover:bg-[#6a8e77] transition">
                  Done
                </button>
              </div>
            ) : (
              <>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <input required placeholder="Your name" value={form.name} onChange={e => setForm({...form, name: e.target.value})}
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#7A9E87]" />
                  ...
                  <input required type="time" value={form.time} onChange={e => setForm({...form, time: e.target.value})}
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#7A9E87]" />
                  <textarea placeholder="Any notes or requests?" value={form.notes} onChange={e => setForm({...form, notes: e.target.value})}
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#7A9E87] h-20 resize-none" />
                  <button type="submit" disabled={submitting}
                    className="w-full bg-[#7A9E87] text-white py-3 rounded-xl hover:bg-[#6a8e77] transition font-medium">
